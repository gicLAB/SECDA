void ACCNAME::VM_PE(
		sc_uint<32>* l1,sc_uint<32>* l2,sc_uint<32>* l3,sc_uint<32>* l4,
		sc_uint<32>* r1,sc_uint<32>* r2,sc_uint<32>* r3,sc_uint<32>* r4,
		sc_uint<32> out[][4],int d, int l_pointer, int wID){
	sc_uint<32> lhs_read[4];
	sc_uint<32> rhs_read[4];
	sc_uint<8> in_a[8];
	sc_uint<8> in_b[8];
	sc_uint<8> we_a[8];
	sc_uint<8> we_b[8];
	sc_uint<32> prod[16][4];
...
#pragma HLS unroll
			...
			prod[i*4+2][1] = mul_u8(in_a[1*4+i],we_a[1*4+2]);
			prod[i*4+3][1] = mul_u8(in_a[1*4+i],we_a[1*4+3]);
			prod[i*4+0][2] = mul_u8(in_b[0*4+i],we_b[0*4+0]);
			prod[i*4+1][2] = mul_u8(in_b[0*4+i],we_b[0*4+1]);
			prod[i*4+2][2] = mul_u8(in_b[0*4+i],we_b[0*4+2]);
			prod[i*4+3][2] = mul_u8(in_b[0*4+i],we_b[0*4+3]);
			prod[i*4+0][3] = mul_u8(in_b[1*4+i],we_b[1*4+0]);
			prod[i*4+1][3] = mul_u8(in_b[1*4+i],we_b[1*4+1]);
			prod[i*4+2][3] = mul_u8(in_b[1*4+i],we_b[1*4+2]);
			prod[i*4+3][3] = mul_u8(in_b[1*4+i],we_b[1*4+3]);
		}
		for(int i=0;i<16;i++){
#pragma HLS unroll
			out[i][0] += prod[i][0];
